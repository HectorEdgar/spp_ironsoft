...
<div class="col-12">
    ...
    <g:hasErrors bean="${this.puesto}">
        <ul class="errors" role="alert">
            <g:eachError bean="${this.puesto}" var="error">
                <li <g:if test="${error in org.springframework.validation.FieldError}">data-field-id="${error.field}"</g:if>><g:message error="${error}"/></li>
            </g:eachError>
        </ul>
    </g:hasErrors>
    <g:form resource="${this.puesto}" method="PUT" autocomplete="off">
        <g:hiddenField name="version" value="${this.puesto?.version}" />
        <fieldset class="form">
            <f:all bean="puesto"/>
        </fieldset>
